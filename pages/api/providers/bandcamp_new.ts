...
// import bcApi from 'bandcamp-fetch';
...
// 			albumData = await bcApi.album.getInfo({albumUrl: url as string});
// 		} else if (url.includes("/track/")) {
// 			albumData = await bcApi.track.getInfo({trackUrl: url as string})
// 		}
// 		return albumData;
//     } catch (error) {
...
//     try {
//         let trackData = await bcApi.track.getInfo({trackUrl: url as string });
//         return trackData;
//     } catch (error) {
...
// 	try {
// 		const data = await bcApi.search.artistsAndLabels({ query: artistName, page: 1 });
// 		if (data) {
// 			return data;
// 		}
// 		return null;
