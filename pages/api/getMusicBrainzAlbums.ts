...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	const api = new ServerAPIHandler('getMusicBrainzAlbums', res, stages, ['mbid', 'offset, limit', 'forceRefresh'])
	try {
		...
		stages.start('Get target artist albums', 'musicbrainz');
		...
		stages.end('Get target artist albums');
		...
		logger.error("Error in getMusicBrainzAlbums API", error);
