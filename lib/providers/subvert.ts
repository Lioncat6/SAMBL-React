        ...
        if (match){
            try {
                ...
                if (entityID) {
                    ...
                    err.handleError(`Failed to locate entity ID`)
                }
            } catch (error) {
                err.handleError(`Failed to parse page props JSON`, error);
            }
        } else {
...
async function getArtistAlbums(id: string, offset: number = 0, limit: number = 100): Promise<SubvertSearchResultsWithArtist | null> {
    try {
        ...
        if (rawArtist) {
            ...
            const data = await subvertFetch('discover/search', { "mode": "fuzzy", "domains": ["release", "track"], "limit": limit, "offset": offset, "query": artist.name });
            if (data && typeof data == "object" && "results" in data) {
                let rawdata = data as SubvertSearchResultsWithArtist;
                let filteredResults = rawdata.results.filter(result => result.artistSlug == typedRawArtist.slug);
...
function formatAlbumGetData(rawData: SubvertSearchResults): RawAlbumData {
    // Filter out tracks with releases 
    const albums = rawData.results.filter(result => "metadata" in result && !result.metadata.release);
    return {
        albums,
...
function formatAlbumObject(rawData: SubvertSearchResultWithArtist | SubvertAlbum | SubvertTrack): AlbumObject {
    if ("metadata" in rawData) {
        ...
        return {
            ...
            type: 'album',
            name: album.name,
            url: createUrl('album', `${album.artistSlug}/${!isRelease ? 'tracks/' : ''}${album.slug}`),
            ...
            trackCount: album.metadata.trackCount ?? tracks.length,
            upc: null,
            albumType: album.metadata.releaseType ?? (isRelease ? null : 'Single'),
            mediums: medium.convertTrackList(tracks.map(formatTrackObject) ?? null, 'Digital Media'),
            labels: null,
            copyrights: null,
        ...
        return {
            ...
            mediums: medium.convertTrackList(album.tracks.map(formatTrackObject), 'Digital Media')
        }
    } else {
        ...
        return {
            ...
            artistNames: album.artists.map((artist) => artist.name),
            trackCount: 1,
            mediums: medium.convertTrackList([formatTrackObject(album)], 'Digital Media')
        }
    }
}
...
function formatTrackObject(rawData: SubvertSearchAlbumTrackWithArtistPosition | SubvertAlbumTrackWithArtistPosition | SubvertTrack | SubvertSearchResultWithArtist): TrackObject {
    ...
    } else if ("metadata" in rawData) {
        const track = rawData as SubvertSearchResultWithArtist;
        return {
            provider: namespace,
            id: track.id,
            name: track.name,
            ...
            isrcs: [],
            type: 'track',
            imageUrl: createSubvertImage(track.imageId),
            imageUrlSmall: createSubvertImage(track.imageId, true)
        }
    } else {
