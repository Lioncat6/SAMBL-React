...
export default async function handler(req, res) {
    ...
    try {
        let data = [];
        if (type === "UPC") {
            const [spotifyData, mbData] = await Promise.all([
                spotify.getAlbumByUPC(query),
                musicbrainz.getAlbumByUPC(query)
            ]);
            ...
            const [spotifyData, mbData] = await Promise.all([
                spotify.getTrackByISRC(query),
                musicbrainz.getTrackByISRC(query)
            ]);
